...
const selectModel = (modelName: string) => {
  ...
  nextTick(() => {
    ...
    // 强制重绘
    void modelSelectorRef.value.offsetHeight
      ...
      <div class="px-4 pt-1.5 pb-1 relative z-20 shrink-0">
        <div class="flex items-center gap-2">
          <div class="model-selector relative inline-block z-[110]">
            ...
            >
              <div
                class="w-5 h-5 rounded-full overflow-hidden bg-muted/50 flex items-center justify-center border border-border/20 group-hover:scale-105 transition-transform duration-300 shrink-0"
              >
                <img
              ...
              <span
                class="text-foreground/80 font-medium text-xs tracking-wide group-hover:text-foreground whitespace-nowrap transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)]"
                >{{ selectedModel }}</span
              >
              <ChevronUp
            ...
            >
              <div
                v-if="showModelMenu"
                class="absolute bottom-full left-0 mb-3 w-60 bg-background/90 backdrop-blur-2xl border-[0.5px] border-border rounded-xl shadow-2xl overflow-hidden origin-bottom-left z-[120]"
              >
                <div class="flex max-h-[48vh] flex-col">
                  <div class="flex-1 overflow-y-auto p-1 space-y-0.5 custom-scrollbar">
                    <button
                    ...
                    >
                      <div class="flex items-center gap-2.5 relative z-10">
                        <div
                          class="w-5 h-5 rounded-full bg-background/50 border border-border/20 overflow-hidden shrink-0 shadow-sm group-hover:scale-105 transition-transform duration-300"
                        >
                          <img
                            :src="model.icon"
                            alt="model icon"
