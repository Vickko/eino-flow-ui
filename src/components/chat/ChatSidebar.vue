  ...
  <div class="flex flex-col h-full bg-background/80 backdrop-blur-xl border-r border-border/40">
      ...
      <div class="relative h-9 mb-4">
        ...
        <Transition name="fade-slide" mode="out-in">
          <h2
            v-if="!collapsed"
            key="title"
            class="absolute left-0 top-1/2 -translate-y-1/2 text-lg font-semibold tracking-tight"
          >
            Messages
          </h2>
        </Transition>
    ...
    <div class="flex-1 overflow-y-auto px-2 pb-2 space-y-1 custom-scrollbar">
      ...
      >
        <!-- Avatar - 始终保持在左侧固定位置 -->
        <div
          class="relative flex-shrink-0 w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary"
        >
          <MessageSquare class="w-5 h-5" />
          <span
            v-if="conv.unreadCount > 0"
            class="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full border-2 border-background flex items-center justify-center text-[10px] text-white font-bold"
          >
            {{ conv.unreadCount }}
          </span>
        </div>
                ...
                <span
                  :class="
                    cn(
                      ...
                      activeId === conv.id ? 'text-foreground' : 'text-foreground/90'
                    )
                  "
                >
                  {{ conv.title }}
      ...
      <div
        v-if="!collapsed && filteredConversations.length === 0"
