...
<template>
  <aside
    class="w-64 h-full rounded-xl border border-border/40 bg-background/60 backdrop-blur-xl flex flex-col shadow-panel overflow-hidden"
  >
    <!-- Header / Branding -->
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex flex-col">
          ...
          <div class="flex items-center gap-1.5 mt-1">
            ...
            ></div>
            <span
              class="text-[10px] font-medium uppercase tracking-wider text-muted-foreground transition-opacity duration-300"
    ...
    <div class="flex-1 overflow-y-auto overflow-x-hidden p-2">
      <div
        class="text-[10px] font-semibold text-muted-foreground px-3 py-2 uppercase tracking-wider opacity-80"
      >
        Graphs
      </div>
    ...
    <div class="p-3 border-t border-border/40 bg-muted/10">
      <div v-if="!isEditingApi" class="group flex flex-col gap-1">
        ...
        <div class="text-xs text-muted-foreground truncate font-mono" :title="apiBaseUrl">
          {{ apiBaseUrl }}
        </div>
      </div>
      ...
      <div v-else class="flex flex-col gap-2">
        ...
        <div class="flex items-center gap-2 justify-end">
          <button
            class="p-1 hover:bg-red-500/10 text-muted-foreground hover:text-red-500 rounded"
            title="Cancel"
            @click="cancelEditingApi"
          >
            <X class="w-3 h-3" />
          </button>
          <button
            class="p-1 hover:bg-green-500/10 text-muted-foreground hover:text-green-500 rounded"
            title="Save"
            @click="saveApiUrl"
          >
            <Check class="w-3 h-3" />
